...
//        findLowestPrice(products,discounts);
    ...
    public static List<User> disturbable(LocalTime now, List<User> users){
        return users.stream()
                .filter(user -> now.isBefore(user.getDndStart()) || now.isAfter(user.getDndEnd()))
                .collect(Collectors.toList());
    }
    public static int solution(List<List<String>> products, List<List<String>> discounts){
        int totalPrice = 0;
        for(int i = 0; i < products.size();i++){
            int price = Integer.valueOf(products.get(i).get(0));
            for(int k = 0; k < products.get(i).size();k++){
                String discountType = products.get(i).get(k);
                List<String> currentDiscount =
                        discounts.stream()
                        .flatMap(e-> e.stream()).filter(elt -> elt.equals(discountType))
                        .collect(Collectors.toList());
                int discountCost = Integer.valueOf(currentDiscount.get(2));

                if(discountType.equals("d0")) {
                    int cost = price - price*discountCost/100;
                    price = Math.min(cost,price);
                }else if(discountType.equals("d1")) {
                    int cost = price - discountCost;
                    price = Math.min(cost,price);
                }
    ...
    public static int findLowestPrice2(List<List<String>> products, List<List<String>> input) {
        int result = 0;
        int totalPrice = 0;
        for(int i = 0; i < products.size();i++) {
            int index = i;
            List<List<String>> discounts = input.stream()
                    .filter(data -> data.get(0).equals(products.get(index))).collect(Collectors.toList());
            totalPrice +=
                    discounts.stream().map(str -> applyDiscount(products.get(index),str))
                            .map(l-> applyDiscount(products.get(index),l)).min().orElse(0);
        }
        return result;
    }
    ...
    public static int findLowestPrice(List<List<String>> products, List<List<String>> discounts) {
        int result = 0;
//        List<String> discountStreams =  discounts.stream().flatMap(e -> e.stream()).collect(Collectors.toList());
        int  totalPrices = products.stream().map(prod -> {
            int price = Integer.valueOf(prod.get(0)); int size = prod.size();
            for(int i = 1; i < size; i++) {
                int index = i;
                 int cost = discounts.stream()
                        .filter(discount -> discount.get(0).equals(prod.get(index)))
                        .mapToInt(l-> applyDiscount(prod,l)).min().orElse(0);
    ...
    public static int applyDiscount(List<String> product, List<String> discount) {
//        List<String> list = discounts.stream().filter(e->e.get(0).equals(product.get(1)))
//                .flatMap(t -> t.stream()).collect(Collectors.toList());

        int price = Integer.valueOf(product.get(0)); int reelPrice = price;
        int discountCost = Integer.valueOf(discount.get(2));
        for(int i = 1; i < product.size(); i++) {
            int cost = price;
            if(product.get(i).equals("d0")) {
                 cost = reelPrice - reelPrice*discountCost/100;
                reelPrice = Math.min(cost,reelPrice);
            }else if(discount.get(0).equals("d1")) {
                cost = reelPrice - discountCost;
                reelPrice = Math.min(cost,reelPrice);
            } else cost = price;
        }
        return (int) Math.round(reelPrice);
    }
